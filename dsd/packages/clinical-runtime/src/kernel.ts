...
import {
  ...
  emitPatientStaleDetected,
  emitRespiratoryCriticalTick,
} from "./scenarios/respiratoryDeterioration.js";
import { initCohortDynamics, planCohortControlledTick, type CohortDynamicsMap } from "./scenarios/patientDynamicState.js";
...
export class ClinicalRuntimeKernel {
  private _state: ClinicalDomainState;
  readonly audit: AppendOnlyAuditWriter;
  readonly telemetry: RuntimeTelemetry;
  private readonly ctx: ReturnType<typeof createRespiratoryScenarioContext>;
  private _lastQueueFingerprint: string | null = null;
  ...
  private applyMany(raws: readonly unknown[]): void {
    for (const raw of raws) {
      const e = this.audit.append(raw);
      this.telemetry.onEventAppended();
      this._state = reduceClinical(this._state, e);
      if (e.type === "VITAL_UPDATED") this.telemetry.onVitalUpdated();
      if (e.type === "DEGRADED_ENTER") this.telemetry.degradedEnterCount++;
      if (e.type === "DEGRADED_EXIT") this.telemetry.degradedExitCount++;
      if (e.type === "STALE_DETECTED") this.telemetry.staleDetectedCount++;
      if (e.type === "STALE_CLEARED") this.telemetry.staleClearedCount++;
    }
  }
   ...
   */
  private flushQueueRecompute(input: {
  ...
  ingestVitalBatch(input: {
    ...
    nowIso: string;
    patientId: string;
    patches: ReadonlyArray<Partial<{ hr: number; spo2: number; sbp: number; lastConfirmedAt: string }>>;
  }): void {
   ...
   */
  runRespiratoryCriticalTick(input: { nowIso: string; tickId: string; forceStale?: boolean }): void {
  ...
  acknowledgeAlert(input: { alertId: string; actor: string; correlationId: string; nowIso: string }): void {
  ...
  signalPatientStale(input: { nowIso: string; tickId: string; correlationId: string }): void {
  ...
  runControlledRandomTick(input: {
    ...
    if (plan.envelopes.length === 0) return plan.dynamics;

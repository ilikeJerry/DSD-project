...
function vitalRiskBoost(patient) {
    if (!patient)
        return 0;
...
function rankAlert(a, patient, nowMs) {
...
export function recomputeQueue(state, nowIso) {
    ...
    for (const a of sorted) {
        ...
        if (p)
            reasons.push(`stale:${staleLevelForAgeMs(nowMs - Date.parse(p.lastConfirmedAt))}`);
        reasonByAlertId.set(a.id, reasons);
    }
    const snapshot = {
        policyVersion: QUEUE_POLICY_VERSION,
        alertIds: sorted.map((a) => a.id),
        patientClock: [...state.patients.entries()].map(([id, p]) => [id, p.lastConfirmedAt]),
    };
...
function hashFromDeterministicJson(snapshot) {
...
//# sourceMappingURL=queueRecompute.js.map
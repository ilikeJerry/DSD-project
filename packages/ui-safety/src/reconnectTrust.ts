...
export const RECONNECT_TRUST_PHASES = ["reconnecting", "syncing", "restored"] as const;
...
export function reconnectBannerPriority(mode: string): number {
  if (mode === "OFFLINE") return 4;
  if (mode === "RECONNECTING") return 3;
  if (mode === "DEGRADED_REALTIME" || mode === "SIMULATION_LAG") return 2;
  return 0;
}

...
export const EventEnvelopeSchema = z
    ...
    if (needsPatient.includes(val.type) && !val.patientId) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "patientId required for this type", path: ["patientId"] });
    }
    const staleNeedsPatient = ["STALE_DETECTED", "STALE_CLEARED"];
    if (staleNeedsPatient.includes(val.type) && !val.patientId) {
    ...
    if (needsAlert.includes(val.type) && !val.alertId) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "alertId required for this type", path: ["alertId"] });
    }
    const needsActor = ["ALERT_ACKED", "ALERT_RESOLVE_REQUESTED", "ALERT_RESOLVED"];
    if (needsActor.includes(val.type) && !val.actor) {
...
export function deterministicSerialize(value) {

...
import {
  ...
  detailRecommendedActionTitle,
  detailSectionTitle,
  detailStaleSelection,
  nextChangeLabel,
  nextOperationalActionLabel,
  resolveDetailPrimaryAction,
...
const DetailCard = memo(function DetailCard({
  ...
  return (
    <div style={dsdDetailCardVariant.base}>
      ...
      <p style={{ margin: "0 0 " + dsdSpace.sm + "px", ...dsdType.title, fontSize: 17 }}>
        {formatPatientPrimaryLine(patient.displayName)}
      </p>
      <div style={dsdDetailActionHighlightStyle()} role="status">
        <p style={{ margin: 0, ...dsdType.section }}>{detailRecommendedActionTitle}</p>
        <p style={{ margin: dsdSpace.xs + "px 0 0", ...dsdType.body, fontWeight: 700, fontSize: 15 }}>{primaryAction}</p>
      </div>
      ...
      >
        ...
        <dd style={{ margin: 0 }}>{patientActionLine(patient.recommendedAction)}</dd>
      </dl>
...
export function FailureSurvivableOperatorStack({
  ...
  const isProduct = presentationMode === "product";
  const degraded = degradedMode !== "HEALTHY";
  ...
  const operatorTimeline = useMemo(
    () => summarizeOperatorTimeline(shell.auditTimeline, 5),
    [shell.auditTimeline],
  );
  ...
  const wardFlow = useMemo(
    () =>
      buildWardFlowNarrative({
        patients: shell.patients,
        alerts: shell.alerts,
        panelConfidence: shell.trust.panelConfidence,
        degradedMode,
      }),
    [shell.patients, shell.alerts, shell.trust.panelConfidence, degradedMode],
  );

  const nextAction =
    openCritical > 0
      ? `미인지 위급 ${openCritical}건 우선 확인`
      : queueLen > 0
        ? "우선순위 큐 상단 환자부터 인지"
        : "병동 감시 유지";
    ...
    >
      ...
      {demoPhase && (
        ...
        >
          ...
          {isProduct ? (
            <>
              <p style={{ margin: dsdSpace.xs + "px 0 0", ...dsdType.bodySm, fontWeight: 600 }}>
                {wardFlowTitle}: {wardFlow}
              </p>
              <p style={{ margin: dsdSpace.xs + "px 0 0", ...dsdType.caption, color: "var(--dsd-text-muted)" }}>
                {currentAttentionLabel}: {shell.trust.staleHierarchySummary}
              </p>
              <p style={{ margin: dsdSpace.xs + "px 0 0", ...dsdType.caption }}>{nextOperationalActionLabel}: {nextAction}</p>
            </>
          ) : (
            <>
              <p style={{ margin: dsdSpace.xs + "px 0 0", ...dsdType.caption, color: "var(--dsd-text-muted)" }}>
              ...
              <p style={{ margin: dsdSpace.xs + "px 0 0", ...dsdType.bodySm, fontWeight: 600 }}>
                {wardFlowTitle}: {demoPhase.narrativeBundle}
              </p>
            </>
          )}
        </div>
      ...
      <div className="dsd-main-grid">
        ...
        <aside style={{ display: "flex", flexDirection: "column", gap: dsdSpace.lg, minWidth: 0 }}>
          <section aria-label={timelineSectionTitle} style={dsdSurfacePanelStyle()}>
            <h3 style={{ margin: "0 0 " + dsdSpace.sm + "px", ...dsdType.section }}>{timelineRecentTitle}</h3>
            {operatorTimeline.length === 0 ? (
              <p style={{ margin: 0, ...dsdType.caption }}>{timelineEmpty}</p>
            ) : (
              <ol style={{ margin: 0, paddingLeft: dsdSpace.lg, ...dsdType.bodySm }}>
                {operatorTimeline.map((label, i) => (
                  <li key={`${label}-${i}`} style={{ marginBottom: dsdSpace.sm }}>
                    {label}
                  </li>
                ))}
              </ol>
            )}
            {shell.auditTimeline.length > operatorTimeline.length && (
              <details style={{ marginTop: dsdSpace.sm }}>
                <summary style={{ ...dsdType.caption, cursor: "pointer" }}>{timelineMoreLabel}</summary>
                <ul style={{ ...dsdType.caption, margin: dsdSpace.sm + "px 0 0", paddingLeft: dsdSpace.lg }}>
                  {shell.auditTimeline.map((e, i) => (
                    <li key={`tl-${i}`}>
                      {e.patientShortName ? `${e.patientShortName}: ` : ""}
                      {auditEventOperatorLabel(e.type)}
                    </li>
                  ))}
                </ul>
                {!isProduct && (
                  <ul style={{ ...dsdType.caption, margin: dsdSpace.xs + "px 0 0", paddingLeft: dsdSpace.lg }}>
                    {shell.auditTailTypes.map((t, i) => (
                      <li key={`raw-${i}`}>{auditEventOperatorLabel(t)}</li>
                    ))}
                  </ul>
                )}
              </details>

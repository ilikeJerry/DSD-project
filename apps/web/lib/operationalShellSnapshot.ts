...
import { buildAuditTimelineEntries, type AuditTimelineEntry } from "./operatorMicrocopy";
...
export type OperationalShellSnapshot = QueuePanelSnapshot & {
  ...
  readonly auditTimeline: readonly AuditTimelineEntry[];
};
...
export function operationalShellSnapshotFromState(
  ...
  for (const [id, p] of state.patients) {
    ...
    patients[id] = {
      ...
      department: profile?.department ?? "WARD",
      stale,
  ...
  const auditTimeline = buildAuditTimelineEntries(auditEntries, patients, 24);
  return { ...base, trust, observability, patients, auditTailTypes: tail, auditTimeline };
}
